...
# def test_downloads(msv4_corpus_metadata):
#   with ExitStack() as stack:
#     pool = stack.enter_context(cf.ThreadPoolExecutor())
#     user_cache_dir = platformdirs.user_cache_dir("xarray-ms", ensure_exists=True)
#     test_data_dir = os.path.join(user_cache_dir, "msv4-test-data")

#     with open(f"{test_data_dir}{os.path.sep}metadata.json", "w") as mf:
#       mf.write(json.dumps(msv4_corpus_metadata, indent=2))

#     futures = []

#     for dataset, metadata in msv4_corpus_metadata["metadata"].items():
#       ds_meta = DatasetMetadata(**metadata)

#       if dataset in IGNORE_DATASETS or ds_meta.type not in DESIRED_DATASET_TYPES:
#         continue
...
#         # or local_filesize != ds_meta.filesize
#       ):
#         f = pool.submit(download_item, ds_meta, ds_archive_file, ds_checksum_file)
#         futures.append(f)
